...
void Robot::bailarParaLaPresentacion(int velocidad) {
  myDFPlayer.play(16);
  bailar(velocidad, 5, 5);
  dibujarCaritaEntusiasmada();
  delay(100);
  girarALaDerechaBailando();
  dibujarCaritaFeliz();
  delay(100);
  bailar(velocidad, 5, 5);
  dibujarCaritaEntusiasmada();
  delay(100);
  girarALaIzquierdaBailando();
  dibujarCaritaFeliz();
  delay(100);
  bailar(velocidad, 5, 5);
  dibujarCaritaEntusiasmada();
...
void Robot::girarALaDerechaBailando() {
  contador0 = 0;
  contador1 = 0;
  //gira a la derecha
  while (contador1 < 15) {
...
void Robot::girarALaIzquierdaBailando() {
  contador0 = 0;
  contador1 = 0;
  //gira a la izquierda
  while (contador0 < 15) {
...
void Robot::bailar(int velocidad, int giroDerecha, int giroIzquierda) {
  girarALaDerechaParaBailar(giroDerecha);
  delay(50);
  girarALaIzquierdaParaBailar(giroIzquierda);
  delay(50);
  girarALaIzquierdaParaBailar(giroIzquierda);
  delay(50);
  girarALaDerechaParaBailar(giroDerecha);
  delay(50);
  moverHaciaAdelanteParaBailar(velocidad);
  delay(50);
  moverHaciaAtrasParaBailar(velocidad);
}

void Robot::girarALaDerechaParaBailar(int giroALaDerecha) {
  contador0 = 0;
  contador1 = 0;
  //gira a la derecha
  while (contador1 < giroALaDerecha) {
      motor0.atras(150);
      motor1.adelante(150);
      delay(15);
      led1.rgb('b');
...
void Robot::girarALaIzquierdaParaBailar(int giroALaIzquierda) {
  contador0 = 0;
  contador1 = 0;
  //gira a la izquierda
  while (contador0 < giroALaIzquierda) {
      motor1.atras(150);
      motor0.adelante(150);
      delay(15);
      led2.rgb('b');
...
void Robot::moverHaciaAdelanteParaBailar(int velocidad) {
  contador0 = 0;
  contador1 = 0;
  //se mueve hacia adelante
  while(contador0 <= 10  && contador1<= 10)
    {
...
void Robot::moverHaciaAtrasParaBailar(int velocidad) {
  contador0 = 0;
  contador1 = 0;
...
void Robot::ejecutarRutinaBT(int velocidad) {
   bool banderaBt = true;
     ...
     if(myBT.available() > 0){
      ...
      Serial.println("Estoy conectado a BT");
       ...
       if (data == 'F'){
        ...
        if (data=='I'){
          //avanzar Derecha
          motor0.adelante(velocidad -80);
          motor1.adelante(velocidad);
        }
        if (data=='G'){
          //avanzar Izquierda
          motor0.adelante(velocidad);
          motor1.adelante(velocidad -80);
        }
        if (data=='J'){
          //retroceder Derecha
          motor0.atras(velocidad);
          motor1.atras(velocidad -80);
        }
        if (data=='H'){
          //retroceder Izquierda
          motor0.atras(velocidad -80);
          motor1.atras(velocidad);
        }
        if (data=='V'){
          //retroceder Izquierda
          bailarParaLaPresentacion(velocidad);
        }
        if (data == 'S'){
          pararRobot();
        }
  ...
  bool banderaBt = true;
   ...
   while(banderaBt == true){
     ...
     if(myBT.available() > 0){
      ...
      if (digitalRead(PIN_RESET) == LOW) {          
          banderaBt = false;
           // Este indica que esta en modo de bluetooth por que si no siempre decia el audio de que este en reset (standBy robot)
          Serial.print("fin de bandera");
          reproducirSonido(17, 1900);
          dibujarCaritaFeliz();
      }
